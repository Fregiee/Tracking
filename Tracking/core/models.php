...
function insertAnActivityLog($pdo, $operation, $branch_id, $address, 
		$developer, $contact_number, $username) {

	$sql = "INSERT INTO activity_logs (operation, branch_id, address, 
		developer, contact_number, username) VALUES(?,?,?,?,?,?)";
...
function insertABranch($pdo, $address, $developer, $contact_number, $added_by) {
	$response = array();
	$sql = "INSERT INTO branches (address, developer, contact_number, added_by) VALUES(?,?,?,?)";
	$stmt = $pdo->prepare($sql);
	$insertBranch = $stmt->execute([$address, $developer, $contact_number, $added_by]);
	...
	if ($insertBranch) {
		...
		$insertAnActivityLog = insertAnActivityLog($pdo, "INSERT", $getBranchID['branch_id'], 
			$getBranchID['address'], $getBranchID['developer'], 
...
function updateBranch($pdo, $address, $developer, $contact_number, 
	...
	$sql = "UPDATE branches
			...
			";
	$stmt = $pdo->prepare($sql);
	$updateBranch = $stmt->execute([$address, $developer, $contact_number, 
	$last_updated, $last_updated_by, $branch_id]);
...
function deleteABranch($pdo, $branch_id) {
	...
	$insertAnActivityLog = insertAnActivityLog($pdo, "DELETE", $getBranchByID['branch_id'], 
		$getBranchByID['address'], $getBranchByID['developer'], 
